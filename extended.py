...
def get1(key):
    ...
    if val is not None:
        ...
        raise ApiException(message="Could not find key '{}' in cache".format(key),
                           status_code=404)
...
def get():
    values = []
    if request.args and request.args.getlist('key'):
...
# def getallkeys():
    ...
    #         val = val.decode('utf-8')
    #         app.logger.debug(val)
    #         json_obj = {'value':str(val)}
    #         values[key].append(json_obj)
    # return jsonify(values=values)
